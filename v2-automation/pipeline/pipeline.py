...
只返回 JSON，不要包含任何其他文字、代码块标记或思考过程。
{{
    ...
    try:
        for i, item in enumerate(items):
            ...
            try:
                ...
                # 解析 LLM 返回的 JSON（鲁棒提取）
                content = response.content.strip()
                # 提取最外层 JSON 对象
                if (js_start := content.find("{")) >= 0 and (js_end := content.rfind("}")) > js_start:
                    content = content[js_start : js_end + 1]
                # 修复未转义的控制字符
                ...
                logger.debug("原始响应: %s", response.content[:500])
                # 解析失败时使用默认值
                enriched = {
                    **item,
